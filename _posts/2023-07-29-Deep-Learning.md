...
![3](http://latex.codecogs.com/svg.latex?tanh(x) = \frac{e^x+e^{-x}}{e^x+e^{-x}})
...
![4](http://latex.codecogs.com/svg.latex?b_{x, y}^{i}=a_{x, y}^{i} /\left(k+\alpha \sum_{j=\max (0, i-n / 2)}^{\min (N-1, i+n / 2)}\left(a_{x, y}^{j}\right)^{2}\right)^{\beta})
...
两个 3x3 的卷积核代替一个 5x5 的卷积核

三个 3x3 的卷积核代替一个 7x7 的卷积核
...
通过将M个 D_k x D_k 的卷积核分别作用于输入的各个通道一次，再用 1x1 的卷积核将M个通道的运算结果以一定的权重加起来

传统的神经网络回将N组M个 D_k x D_k 卷积核作用于输入的M个通道一次，然后直接将M个卷积结果加起来
...
![image-4](/assets/img/image-4.png)
...
![image-8](/assets/img/image-8.png)
...
![image-9](/assets/img/image-9.png)

将边框作为一个回归问题，直接卷积。选择直接用卷积神经网络来输出预测边框与预测类别准确度

最终输出是一个 7x7x30 的三维矩阵

![image-10](/assets/img/image-10.png)

将整个输入图片分为 S x S(S=7)个网格grid cell，每个网格对应一个30维的向量，前10维为这个grid cell上生成的两个bounding box的五个参数，后20维是该grid cell对应20个类别的条件概率

根据输出的 49x2=98 个bounding box与类别概率，进行非极大值抑制 non-maximum suppression，删除冗余边界框
...
![image-11](/assets/img/image-11.png)
...
输入大小: W x W, kernel_size=F, stride=S, padding=P

输出大小: ![5](http://latex.codecogs.com/svg.latex?N = \frac{W-F+2P}{S}+1)
